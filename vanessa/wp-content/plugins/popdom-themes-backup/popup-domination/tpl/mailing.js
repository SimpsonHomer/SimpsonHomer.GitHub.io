;(function($){
	$(document).ready(function(){
		...
		if(cur_hash == '' || cur_hash == ' '){
			if(typeof provider != 'undefined' && provider != '' && provider!= ' '){
				$('#popup_domination_tabs a').each(function(){
					listalt = $(this).attr('data-provider');
        ...
        function custom_num() {
            var customnum = parseInt($('.custom_num').val());
            if(customnum < 1){
        ...
        function cc_custom_select() {
            $('#cc_custom2').hide();
            var num = $('#cc_custom_select').val();
            var i = 1;
            while(i<=num){
                $('#cc_custom'+i).show();
                i++;
		...
		$('.getlist').click(function(){
			...
			var provider = $('.tab-menu .selected').attr('data-provider');
