...
(function($) {
    ...
    $(document).ready(function() {
        ...
        $('body').on("click", ".toggle-all", function() {
            var checkBoxes = $(".page_list input:checkbox");
        ...
        function check_themeclub_box() {
            if($('#popdom_contentbox_unlock_themeclub')) {
                var theme_clubs = ['lightbox-monthy-plain', 'lightbox-monthly-multi-field', 'lightboxnew1', 'lightboxnew2', 'lightboxnew3', 'lightboxnew4', 'lightbox-premium', 'simplify-1', 'social-2', 'social-3', 'social-vid', 'lightbox1-redirect'];
                if(theme_clubs.indexOf( $('#popup_domination_template').val() ) != -1) {
                    $('#popdom_contentbox_unlock_themeclub').css('display', 'block');
                } else {
                    $('#popdom_contentbox_unlock_themeclub').css('display', 'none');
                }
            }            
        }
        ...
        var template_org_val = $('#popup_domination_template').val();
        $('#popup_domination_template').change(function() {
            if($(this).val() == 'get_more_themes_link') {
                $(this).val(template_org_val);
                var win = window.open('?page=popup-domination/theme_upload', '_blank');
                win.focus();
            } else {
                template_org_val = $('#popup_domination_template').val();
        ...
        $('body').on('click', '#popup_domination_listitems a[href$="#delete"]', function() {
            $(this).parent().remove();
            refresh_list_icons();
            return false;
        });
        $('#popup_domination_email_box, #popup_domination_name_box').change(function() {
            check_select(this)
        });
        $('#popup_domination_tab_schedule a#clear-cookies').click(function() {
        ...
        $('#popup_domination_tab_schedule .show_opts input[type="radio"]').change(function() {
            $('#popup_domination_tab_schedule .show_opts input[type="radio"]').each(function() {
        ...
        $('#submissions_tab #mailing_list').change(function() {
            var selected = $(this).val();
            if (selected == -1) {
                $('#submissions_tab #redirect_option').show();
            } else {
                $('#submissions_tab #redirect_option').hide();
            }
        });
    ...
    function set_max_msg(obj) {
        ...
        var length = 0;
        if ($(fieldtype, obj).val()) {
            length = $(fieldtype, obj).val().length;
        }
        var elem;
