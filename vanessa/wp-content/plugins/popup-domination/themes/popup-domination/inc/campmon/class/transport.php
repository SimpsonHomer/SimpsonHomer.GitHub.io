...
class CS_REST_CurlTransport extends CS_REST_BaseTransport {
    ...
    function make_call($call_options) {
        ...
        if(!$response && $response !== '') {
            $this->_log->log_message('Error making request with curl_error: ' . curl_errno($ch), get_class($this), CS_REST_LOG_ERROR);
        ...
        $this->_log->log_message('API Call Info for ' . $call_options['method'] . ' ' .
                ...
                ' Total time (seconds): ' . curl_getinfo($ch, CURLINFO_TOTAL_TIME), get_class($this), CS_REST_LOG_VERBOSE);
