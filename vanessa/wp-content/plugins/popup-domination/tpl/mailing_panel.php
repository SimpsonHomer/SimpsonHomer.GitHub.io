...
<div class="wrap with-sidebar" id="popup_domination">
    ...
    <div id="popup_domination_container" class="has-left-sidebar">
        ...
        <div id="popup_domination_tabs" class="campaign-details">
            <div class="campaign-name-box">
                ...
                <input id="landingpage" name="landingpagecheck" type="checkbox" <?PHP if($redirectcheck == 'Y') { echo 'checked="checked"'; } ?>/>
                <div class="clear"></div>
                <p class="microcopy">Leave un-ticked to not re-direct after opt in.</p>
            </div>
            <div class="campaign-description">
                <label for="campdesc">Re-direct URL: </label>
                <input id="landingurl" type="text" name="landingurldata" <?PHP if($redirectcheck != 'Y') { echo 'disabled="disabled"'; } ?> value="<?PHP if($redirectcheck == 'Y') { echo $redirecturl; } ?>" />
                    ...
                    <div class="popdom-inner-sidebar">
                        <div class="provider_divs">
                            ...
                            <div class="aw">
                                ...
                                <a href="<?PHP echo $this->plugin_url . "inc/aweber.php?path=" . $this->plugin_path . '&url=http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']; ?>" alt='aw_apikey' class="connect-to getlist"><span>Connect to Aweber</span></a>
                                <?PHP } else { ?>
                ...
                <div class="inside twodivs">
                    ...
                    <div class="popdom-inner-sidebar">
                        ...
                        <div class="ic">
                            ...
                            <input type="text" name="ic_password" alt='ic' value="<?PHP
                            if(!empty($apidata) && $apidata['provider'] == 'ic') { echo $apidata['password']; } else { echo 'Your Password'; } ?>" id="ic_password" />
                            <span class="example">iContact Username</span>
                            <input type="text" name="ic_username" alt='ic' value="<?PHP
                            if(!empty($apidata) && $apidata['provider'] == 'ic') { echo $apidata['username']; } else { echo 'Your Username'; } ?>" id="ic_username" />
                            <span class="example">iContact Application AppID</span>
                            <input type="text" name="ic_apikey" alt='ic' value="<?PHP if(!empty($apidata) && $apidata['provider'] == 'ic') { echo $apidata['apikey']; } else { echo 'Your App-ID'; } ?>" id="ic_apikey" />
                ...
                <div class="inside twodivs">
                    ...
                    <div class="popdom-inner-sidebar">
                        ...
                        <div class="cm">
                            ...
                            <input type="text" name="cm_clientid" alt='cm' value="<?PHP if(!empty($apidata) && $apidata['provider'] == 'cm') { echo $apidata['apiextra']; } else { echo 'Enter Your Client Id..'; } ?>" id="cm_clientid" />
                            <span class="example">Campaign Monitor API key</span>
                            <input type="text" name="cm_apikey" alt='cm' value="<?PHP if(!empty($apidata) && $apidata['provider'] == 'cm') { echo $apidata['apikey']; } else { echo 'Enter Your Api key'; } ?>" id="cm_apikey" />
                            <h3>Please Select a Mailing List:</h3>
                ...
                <div class="inside twodivs">
                    ...
                    <div class="popdom-inner-sidebar">
                        ...
                        <div class="gr">
                            ...
                            <input type="text" name="gr_apikey" alt='gr' value="<?PHP
                            if(!empty($apidata) && $apidata['provider'] == 'gr') { echo $apidata['apikey']; } else { echo 'Enter Your Api key'; } ?>" id="gr_apikey" />
                            <h3>Please Select a Mailing List:</h3>
                ...
                <div class="inside twodivs">
                    ...
                    <div class="popdom-inner-sidebar">
                        ...
                        <div class="mc">
                            ...
                            <input type="text" name="mc_apikey" alt='mc' id="mc_apikey" value="<?PHP if(!empty($apidata) && $apidata['provider'] == 'mc') { echo $apidata['apikey']; } else { echo 'Enter Your Api key'; } ?>" />
                            <h3>Please Select a Mailing List:</h3>
                ...
                <div class="inside twodivs">
                    ...
                    <div class="popdom-inner-sidebar">
                        ...
                        <div class="nm">
                            ...
                            <input type="text" name="nm_emailadd" alt='nm' id="nm_emailadd" value="<?PHP if(!empty($apidata) && $apidata['provider'] == 'nm') { echo $apidata['apikey']; } else { echo 'Enter Your Email Address'; } ?>" />
                        </div>
                        <span class="example">How Many Extra Fields Would You Like? (this is limited by the template)</span>
                        ...
                        <div class="other">
                            <form action="<?PHP echo $this->opts_url ?>" method="post">
                                ...
                                <div class="col">
                                    ...
                                    <p>
                                        <input type="checkbox" name="popup_domination[new_window]" id="popup_domination_new_window" value="Y"<?PHP $new_window = $this->option('new_window'); echo ($new_window && $new_window == 'Y') ? ' checked="checked"' : ''; ?> />
                                        <label for="popup_domination_new_window">Submit the form to a new window</label></p>
                                    <p>
                                        <input type="checkbox" name="popup_domination[disable_name]" id="popup_domination_disable_name" value="Y"<?PHP $disable_name = $this->option('disable_name'); echo ($disable_name && $disable_name == 'Y') ? ' checked="checked"' : ''; ?> />
                ...
                <div class="inside twodivs">
                    <div class="popdom-inner-sidebar">
                        <form id="form" name="apidata" id="apiformdata" action="<?PHP echo $this->opts_url ?>" method="post">
                            ...
                            <input type="hidden" name="oldlistid" class="oldlistid" value="<?PHP if(!empty($apidata)) { echo $apidata['listsid']; } ?>"/>
                            <input type="hidden" name="provider" class="provider" value="<?PHP if(!empty($apidata)) { echo $apidata['provider']; } ?>"/>
                            <input type="hidden" name="apikey" class="apikey" value="<?PHP if(!empty($apidata)) { echo $apidata['apikey']; } ?>"/>
                            <input type="hidden" name="username" class="username" value="<?PHP if(!empty($apidata)) { echo $apidata['username']; } ?>"/>
                            <input type="hidden" name="password" class="password" value="<?PHP if(!empty($apidata)) { echo $apidata['password']; } ?>"/>
                            <input type="hidden" name="apiextra" class="apiextra" value="<?PHP if(!empty($apidata)) { echo $apidata['apiextra']; } ?>"/>
                            <input type="hidden" name="redirecturl" class="redirecturl" value="<?PHP if(!empty($redirecturl)) { echo $redirecturl; } ?>"/>
                            <input type="hidden" name="listname" class="listname" value="<?PHP if(!empty($apidata)) { echo $apidata['listname']; } ?>" />
                            <input type="hidden" name="customf" class="customf" value="<?PHP if(!empty($apidata)) { echo $apidata['customf']; } ?>"/>
                            <span class="example custom1" style="<?PHP if(isset($apidata['custom1']) && !empty($apidata['custom1'])) {
    echo 'display:block;';
} else {
    echo 'display:none;';
} ?>">What is Your 1st Custom Field Name? (Need Help? <a target="_blank" href="http://popdom.desk.com/customer/portal/articles/367583-extra-custom-fields">Click Here</a>)</span>
                            <input type="text" style="<?PHP if(isset($apidata['custom1']) && !empty($apidata['custom1'])) {
    echo 'display:block;';
} else {
    echo 'display:none;';
} ?>" name="custom1" class="custom1" value="<?PHP if(!empty($apidata)) { echo $apidata['custom1']; } ?>"/>
                            <span class="example custom2" style="<?PHP if(isset($apidata['custom2']) && !empty($apidata['custom2'])) {
    echo 'display:block;';
} else {
    echo 'display:none;';
} ?>">What is Your 2nd Custom Field Name? (Need Help? <a target="_blank" href="http://popdom.desk.com/customer/portal/articles/367583-extra-custom-fields">Click Here</a>)</span>
                            <input type="text" style="<?PHP if(isset($apidata['custom2']) && !empty($apidata['custom2'])) {
    echo 'display:block;';
} else {
    echo 'display:none;';
} ?>" name="custom2" class="custom2" value="<?PHP if(!empty($apidata)) { echo $apidata['custom2']; } ?>"/>
    ...
    <script type="text/javascript">
        var popup_domination_admin_ajax = '<?PHP echo admin_url('admin-ajax.php') ?>',
            popup_domination_theme_url = '<?PHP echo $this->theme_url ?>',
            popup_domination_form_url = '<?PHP echo $this->opts_url ?>',
            popup_domination_url = '<?PHP echo $this->plugin_url ?>',
            website_url = '<?PHP echo site_url(); ?>',
            numfields = '<?PHP echo $number; ?>',
            provider = '<?PHP if(isset($apidata['provider']) && !empty($apidata['provider'])) { echo $apidata['provider']; } else { echo 'other'; } ?>',
    </script>