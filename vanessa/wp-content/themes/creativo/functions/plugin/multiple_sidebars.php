...
class sidebar_generator {
	...
	function admin_menu(){
		add_theme_page(__('Sidebars','Creativo'), __('Sidebars Page','Creativo'), 'manage_options', 'multiple_sidebars', array('sidebar_generator','admin_page'));
	...
	function admin_page(){
		...
		<div class="wrap">
			<h2><?php _e('Sidebars Settings','Creativo');?></h2>
			<br />
			<table class="widefat page" id="sbg_table" style="width:600px;">
				<tr>
					<th><?php _e('Sidebar Name','Creativo');?></th>
					<th><?php _e('CSS class','Creativo');?></th>
					<th><?php _e('Remove','Creativo');?></th>
				</tr>
				<?php
				$sidebars = sidebar_generator::get_sidebars();
				...
				<tr class="<?php echo $alt?>">
					...
					<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo $sidebar; ?>',<?php echo $cnt+1; ?>);" title="<?php _e('Remove this sidebar','Creativo');?>"><?php _e('remove','Creativo');?></a></td>
				</tr>
				<?php
						$cnt++;
					}
					...
					?>
					<tr>
						<td colspan="3"><?php _e('No Sidebars defined','Creativo');?></td>
					</tr>
					<?php
				}
				?>
            ...
            <div class="add_sidebar">
				<a href="javascript:void(0);" onclick="return add_sidebar_link()" title="Add a sidebar" class="button-primary"><?php _e('+ Add New Sidebar','Creativo');?></a>
	...
	*/
	...
	<div id='sbg-sortables' class='meta-box-sortables'>
		<div id="sbg_box" class="postbox " >
			<div class="handlediv" title="Click to toggle"><br /></div><h3 class='hndle'><span><?php _e('Sidebar','Creativo');?></span></h3>
			<div class="inside">
				<div class="sbg_container">
					<input name="sbg_edit" type="hidden" value="sbg_edit" />					
					<p>
						<?php _e('Please select the sidebar you would like to display on this page. <strong>Note:</strong> You must first create the sidebar under Appearance > Sidebars.','Creativo'); ?>
					</p>
					<ul>
					<?php 
					...
					//var_dump($wp_registered_sidebars);		
						for($i=0;$i<1;$i++){ ?>
							...
							<select name="sidebar_generator[<?php echo $i?>]" style="display: none;">
								<option value="0"<?php if($selected_sidebar[$i] == ''){ echo " selected";} ?>><?php _e('WP Default Sidebar','Creativo'); ?></option>
							<?php
							...
							<select name="sidebar_generator_replacement[<?php echo $i?>]">
								<option value="0"<?php if($selected_sidebar_replacement[$i] == ''){ echo " selected";} ?>><?php _e('None','Creativo'); ?></option>
							<?php
