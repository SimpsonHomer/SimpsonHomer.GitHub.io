...
window.VcDivider = vc.shortcode_view.extend({
        changeShortcodeParams:function (model) {
            var params = model.get('params');
            window.VcDivider.__super__.changeShortcodeParams.call(this, model);
            if (_.isObject(params) && _.isString(params.style)) {
                this.$el.find('> .wpb_element_wrapper').removeClass(_.values(this.params.style.value).join(' ')).addClass(params.style);
            }
        }
    });
