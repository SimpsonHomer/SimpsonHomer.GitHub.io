...
class WPBakeryShortCode_VC_Posts_Grid extends WPBakeryShortCode {
    ...
    public function jsComposerEditPage() {
        ...
        foreach ($pt_array as $pt) {
            add_meta_box( 'vc_teaser', __('VC: Custom Teaser', "Creativo"), Array(&$this, 'outputTeaser'), $pt, 'side');
        }
    ...
    public function outputTeaser() {
        ...
        wp_localize_script( 'wpb_jscomposer_teaser_js', 'i18nVcTeaser', array(
            'empty_title' => __('Empty title', "Creativo"),
            'text_label' => __('Text', "Creativo"),
            'image_label' => __('Image', "Creativo"),
            'title_label' => __('Title', "Creativo"),
            'link_label' => __('Link', "Creativo"),
            'text_text' => __('Text', "Creativo"),
            'text_excerpt' => __('Excerpt', "Creativo"),
            'text_custom' => __('Custom', "Creativo"),
            'image_featured' => __('Featered', "Creativo"),
            'image_custom' => __('Custom', "Creativo"),
            'link_label_text' => __('Link text', "Creativo"),
            'no_link' => __('No link', "Creativo"),
            'link_post' => __('Link to post', "Creativo"),
            'link_big_image' => __('Link to big image', "Creativo"),
            'add_custom_image' => __('Add custom image', "Creativo")
        ));
        $output = '<div class="vc-teaser-switch"><label><input type="checkbox" name="'.self::$meta_data_name.'[enable]" value="1" id="vc-teaser-checkbox"'.($this->getTeaserData('enable') === '1' ? ' checked="true"' : '').'> '.__('Enable custom teaser', "Creativo").'</label></div>';
    ...
    protected function getLinked($post, $content, $type, $css_class) {
        ...
        if($type === 'link_post') {
            ...
            $title =  sprintf( esc_attr__( 'Permalink to %s', "Creativo" ), $post->title_attribute);
