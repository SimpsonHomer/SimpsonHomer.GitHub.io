...
class WPBakeryVisualComposerSetup extends WPBakeryVisualComposerAbstract {
    ...
    public function addUpgradeMessageLink() {
        ...
        if(empty($username) || empty($api_key) || empty($purchase_code)) {
            echo ' <a href="'.$this->url.'">'.__('Dowload new version from CodeCanyon.', 'Creativo').'</a>';

        } else {
            echo '<a href="'.wp_nonce_url( network_admin_url('options-general.php?page=wpb_vc_settings&vc_action=upgrade')).'">'.__('Update Visual Composer now.', 'Creativo').'</a>';
        }
    }
...
class WPBakeryVisualComposerSetupAdmin extends WPBakeryVisualComposerSetup {
    ...
    public function adminNoticeNewVersion() {
        ...
        if(!empty($version))
            echo '<div class="updated"><p>'.sprintf(__('There is a new version of Visual Composer available. <a href="http://bit.ly/vcomposer" target="_blank">Download version <b>%s</b></a>.', 'Creativo'), $version).'</p></div>';
    }
    ...
    public function registerJavascript() {
        ...
        wp_localize_script( 'wpb_js_composer_js_view', 'i18nLocale', array(
            'add_remove_picture' => __( 'Add/remove picture', 'Creativo' ),
            'finish_adding_text' => __( 'Finish Adding Images', 'Creativo' ),
            'add_image' => __( 'Add Image', 'Creativo' ),
            'add_images' => __( 'Add Images', 'Creativo' ),
            'main_button_title' => __( 'Visual Composer', 'Creativo' ),
            'main_button_title_revert' => __( 'Classic editor', 'Creativo' ),
            'please_enter_templates_name' => __('Please enter templates name', 'Creativo'),
            'confirm_deleting_template' => __('Confirm deleting "{template_name}" template, press Cancel to leave. This action cannot be undone.', 'Creativo'),
            'press_ok_to_delete_section' => __('Press OK to delete section, Cancel to leave', 'Creativo'),
            'drag_drop_me_in_column' => __('Drag and drop me in the column', 'Creativo'),
            'press_ok_to_delete_tab' => __('Press OK to delete "{tab_name}" tab, Cancel to leave', 'Creativo'),
            'slide' => __('Slide', 'Creativo'),
            'tab' => __('Tab', 'Creativo'),
            'section' => __('Section', 'Creativo'),
            'please_enter_new_tab_title' => __('Please enter new tab title', 'Creativo'),
            'press_ok_delete_section' => __('Press OK to delete "{tab_name}" section, Cancel to leave', 'Creativo'),
            'section_default_title' => __('Section', 'Creativo'),
            'please_enter_section_title' => __('Please enter new section title', 'Creativo'),
            'error_please_try_again' => __('Error. Please try again.', 'Creativo'),
            'if_close_data_lost' => __('If you close this window all shortcode settings will be lost. Close this window?', 'Creativo'),
            'header_select_element_type' => __('Select element type', 'Creativo'),
            'header_media_gallery' => __('Media gallery', 'Creativo'),
            'header_element_settings' => __('Element settings', 'Creativo'),
            'add_tab' => __('Add tab', 'Creativo'),
            'are_you_sure_convert_to_new_version' => __('Are you sure you want to convert to new version?', 'Creativo'),
            'loading' => __('Loading...', 'Creativo'),
            // Media editor
            'set_image' => __('Set Image', 'Creativo'),
            'are_you_sure_reset_css_classes' => __('Are you sure taht you want to remove all your data?', 'Creativo'),
            'loop_frame_title' => __('Loop settings'),
            'enter_custom_layout' => __('Enter custom layout for your row:', 'Creativo'),
            'wrong_cells_layout' => __('Wrong row layout format! Example: 1/2 + 1/2 or span6 + span6.', 'Creativo'),
			'row_background_color' => __('Row background color', 'Creativo'),
            'row_background_image' => __('Row background image', 'Creativo')
        ) );
