...
$tabs_nav = '';
$tabs_nav .= '<ul id="tabs" class="tabs">';
foreach ( $tab_titles as $tab ) {
    preg_match('/title="([^\"]+)"(\stab_id\=\"([^\"]+)\"){0,1}/i', $tab[0], $tab_matches, PREG_OFFSET_CAPTURE );
    if(isset($tab_matches[1][0])) {
        $tabs_nav .= '<li class="border_'.$color.'"><a href="#tab-'. (isset($tab_matches[3][0]) ? $tab_matches[3][0] : sanitize_title( $tab_matches[1][0] ) ) .'" class="button_'.$color.'">' . $tab_matches[1][0] . '</a></li>';
...
$output .= "\n\t".'<div class="tab-holder shortcode-tabs '.$css_class.'" data-interval="'.$interval.'">';
$output .= "\n\t\t".'<div class="tab-hold tabs-wrapper">';
$output .= wpb_widget_title(array('title' => $title, 'extraclass' => $element.'_heading'));
$output .= "\n\t\t\t".$tabs_nav;
$output .= "\n\t\t\t".'<div class="tab-containe border_'.$color.'">'.wpb_js_remove_wpautop($content).'</div>';
